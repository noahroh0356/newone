...
public class CameraManager : MonoBehaviour
{
    ...
    void SetCameraSizeByAspect()
    {
        ...
        targetSize = (aspect < 0.8f) ? 9.7f : 10f;
    }

    private void LateUpdate()
    {
        // 카메라 크기 부드럽게 조정
        if (!isMoving && Mathf.Abs(Camera.main.orthographicSize - targetSize) > 0.01f)
        {
    ...
    private void Update()
    {
        ...
        else if (Input.GetMouseButtonUp(0))
        {
            ...
            {
                if (swipeDistanceX > 0 && currentArea == "Restaurant")
                {
                    MoveTo("Kitchen");
                }
                else if (swipeDistanceX < 0 && currentArea == "Kitchen")
                {
                    MoveTo("Restaurant");
                }
            }
    ...
    IEnumerator CoMoveTo(Vector3 targetPosition, string areaName)
    {
        isMoving = true;

        // z축 유지
        targetPosition.z = cameraTr.position.z;

        while (Vector3.Distance(cameraTr.position, targetPosition) > 0.01f)
        {
            cameraTr.position = Vector3.MoveTowards(cameraTr.position, targetPosition, Time.deltaTime * 30f);
            yield return null;
        }
